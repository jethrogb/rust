...
//!   input (`UnsafeListEntry::deref_ptr`), and pointers are converted back to
...
//!   `DerefPtr` (obtained from `UnsafeListEntry::deref_ptr` or
...
impl<T> UnsafeListEntry<T> {
    fn dummy() -> Self {
        UnsafeListEntry { next: EntryPtr::dangling(), prev: EntryPtr::dangling(), value: None }
    }
    ...
    fn deref_ptr(&mut self) -> DerefPtr<'_, T> {
        // SAFETY: the pointer is derived from the exclusive reference to the
        // entry, which the returned `DerefPtr` borrows for its lifetime, so
        // it is valid for dereferences while the result lives.
        unsafe { DerefPtr::new(self.into()) }
    }
}
...
mod ptr {
    ...
    pub(super) struct EntryPtr<T>(NonNull<UnsafeListEntry<T>>);
    ...
    impl<T> From<&mut UnsafeListEntry<T>> for EntryPtr<T> {
        fn from(entry: &mut UnsafeListEntry<T>) -> Self {
    ...
    /// obtained from `UnsafeListEntry::deref_ptr`, or the list for a pointer
    ...
    pub(super) struct DerefPtr<'a, T> {
        ptr: NonNull<UnsafeListEntry<T>>,
        borrow: PhantomData<&'a mut UnsafeListEntry<T>>,
    }
    ...
    impl<'a, T> DerefPtr<'a, T> {
        ...
        /// `UnsafeListEntry::deref_ptr` and `UnsafeList::launder`).
        pub(super) unsafe fn new(ptr: EntryPtr<T>) -> Self {
            DerefPtr { ptr: ptr.0, borrow: PhantomData }
        }

        pub(super) fn as_ptr(&self) -> *mut UnsafeListEntry<T> {
            self.ptr.as_ptr()
        }
...
impl<T> UnsafeList<T> {
    ...
    fn head_tail(&mut self) -> &mut UnsafeListEntry<T> {
        let head_tail = self.head_tail;
        let mut ptr = NonNull::from(self).cast::<UnsafeListEntry<T>>().with_addr(head_tail);
