...
pub fn bind_stream(addr: &str) -> IoResult<(Fd, String)> {
    unsafe {
        ...
        let fd = raw::bind_stream(addr_user.as_ptr(), addr_user.len(), local.as_raw_mut_ptr()).from_sgx_result()?;
...
pub fn accept_stream(fd: Fd) -> IoResult<(Fd, String, String)> {
    unsafe {
        ...
        let bufs: &mut alloc::UserRef<[ByteBuffer]> = &mut *bufs; // FIXME: can this be done without forcing coercion?
        let local = &mut bufs[0];
        let peer = alloc::UserRef::from_mut_ptr(local.as_raw_mut_ptr().offset(1)); // FIXME: use split_at_mut or so
        let fd = raw::accept_stream(fd, local.as_raw_mut_ptr(), peer.as_raw_mut_ptr()).from_sgx_result()?;
...
pub fn connect_stream(addr: &str) -> IoResult<(Fd, String, String)> {
    unsafe {
        ...
        let local = &mut bufs[0];
        let peer = alloc::UserRef::from_mut_ptr(local.as_raw_mut_ptr().offset(1)); // FIXME: use split_at_mut or so
        let fd = raw::connect_stream(addr_user.as_ptr(), addr_user.len(), local.as_raw_mut_ptr(), peer.as_raw_mut_ptr()).from_sgx_result()?;
...
fn check_os_error(err: Result) -> i32 {
    // TODO: not sure how to make sure all variants of Error are covered
