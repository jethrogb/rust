...
impl WaitQueue {
    ...
    pub fn wait<T>(mut guard: SpinMutexGuard<WaitVariable<T>>) {
        unsafe {
            ...
            while !entry.lock().wake {
                assert_eq!(usercalls::wait(EV_UNPARK, WAIT_INDEFINITE).unwrap() & EV_UNPARK, EV_UNPARK);
            }
        }
    }
    ...
    pub fn notify_one<T>(mut guard: SpinMutexGuard<WaitVariable<T>>) -> Result<WaitGuard<T>, SpinMutexGuard<WaitVariable<T>>> {
        unsafe {
    ...
    pub fn notify_all<T>(mut guard: SpinMutexGuard<WaitVariable<T>>) -> Result<WaitGuard<T>, SpinMutexGuard<WaitVariable<T>>> {
        unsafe {
            let mut count = 0;
...
// TODO: Perhaps use Intel TSX to avoid locking?
mod spin_mutex {
    use cell::UnsafeCell;
